import ob_pkg::*;
module flb_refill_engine_bid_1024(
    input logic                           i_clk,
    input logic                           i_rst_n,
...
localparam int GROUP_COUNT = (NUM_LEVELS + 31) / 32;
localparam int GROUP_IDX_LEN = (GROUP_COUNT <= 1) ? 1 : $clog2(GROUP_COUNT);

logic [NUM_LEVELS-1:0]mask_data0, mask_data1;
logic [31:0] grouped_data0, grouped_data1;
logic found_group1, found_inner2, found_inner3;
logic [4:0] inner_idx2;
logic [GROUP_IDX_LEN-1:0] group_idx1, group_idx2;
logic [V_TABLE_IDX-1:0] idx;
logic [31:0] selected_group1, selected_group2;
...
always_comb begin
    ...
    grouped_data0 = '0;
    for(int i = 0; i < GROUP_COUNT; i++) begin
        grouped_data0[i] = |mask_data0[i*32+:32];
    end
end
...
always_comb begin
    ...
    if(found_group1) begin
        selected_group1 = mask_data1[group_idx1*32+:32];
    end
end
...
always_ff @(posedge i_clk, negedge i_rst_n) begin
    if(!i_rst_n) begin
        ...
        idx <= V_TABLE_IDX'((group_idx2 * 32) + inner_idx2);
        epoch3 <= epoch2;
    end
end
