...
module parser_avalon_wrapper #(
    ...
    parameter int AAPL_BOOK_BASE_PRICE = 32'd5_804_000,
    ...
    logic [STOCK_LEN-1:0]          last_order_payload_stock_id;
    ...
    function automatic [31:0] base_price_for_stock(
        input logic [STOCK_LEN-1:0] stock_id_in
    );
        begin
            case (stock_id_in)
                STOCK_LEN'(1): base_price_for_stock = INTC_BOOK_BASE_PRICE;
                STOCK_LEN'(2): base_price_for_stock = MSFT_BOOK_BASE_PRICE;
                STOCK_LEN'(3): base_price_for_stock = AAPL_BOOK_BASE_PRICE;
                default:       base_price_for_stock = BOOK_BASE_PRICE;
            endcase
        end
    endfunction
    ...
    hft_multi_stock_top #(
        ...
        .i_kill_switches      (kill_switch_regs),
    ...
    //   14    : controls [0]=trading_en [1]=kill_stock1 [2]=kill_stock2
    //           [3]=kill_stock3 [4]=price_band_en [5]=pnl_check_en
    ...
    always_ff @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            ...
            if (avs_write) begin
                case (avs_address)
                    ...
                    6'd14: begin
                        ...
                        kill_switch_regs      <= avs_writedata[3:1];
                        price_band_enable_reg <= avs_writedata[4];
                        pnl_check_enable_reg  <= avs_writedata[5];
                    end
            ...
            if (order_payload_valid) begin
                ...
                last_order_payload_stock_id <= stock_id;
    ...
    always_comb begin
        ...
        if (avs_read) begin
            case (avs_address)
                ...
                6'd14: avs_readdata = {26'd0, pnl_check_enable_reg, price_band_enable_reg,
                                       kill_switch_regs[2], kill_switch_regs[1],
                                       kill_switch_regs[0], trading_enable_reg};
                ...
                6'd63: begin
                    case (debug_select_reg)
                        6'd0:  avs_readdata = base_price_for_stock(stock_id);
                        ...
                        6'd20: avs_readdata = {{(32-STOCK_LEN){1'b0}}, last_order_payload_stock_id};
