...
#define TCP_RECEIVER_BUILD_ID "three-stock-killswitch-2026-04-30"
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        *(h2p_lw_parser_addr + 22) = 1;  // pulse soft_rst_n low for 1 cycle
        ...
        unsigned long last_button_state = 0x0F; // KEY[2:0] active-low, SW bits may be either state
        unsigned long last_kill_switch_state = 0xFF;
        ...
        while (1) {
            ...
            // 2. Poll FPGA keys / mode switch / per-stock kill switches.
            // button_pio exports {SW[9], SW[8], SW[7], SW[0], KEY[2:0]}:
            //   bit[6] = SW9 -> stock 1 (INTC) kill switch
            //   bit[5] = SW8 -> stock 2 (MSFT) kill switch
            //   bit[4] = SW7 -> stock 3 (AAPL) kill switch
            //   bit[3] = SW0 -> stream mode switch (not active-low)
            //   bit[2:0] = KEY2..KEY0 (active-low push buttons)
            unsigned long current_button_state = *h2p_lw_button_addr & 0x7F;
            unsigned long kill_stock_1 = (current_button_state >> 6) & 0x1;
            unsigned long kill_stock_2 = (current_button_state >> 5) & 0x1;
            unsigned long kill_stock_3 = (current_button_state >> 4) & 0x1;
            unsigned long current_kill_switch_state =
                (kill_stock_1 << 2) | (kill_stock_2 << 1) | kill_stock_3;
            unsigned long parser_control_word =
                0x31 | (kill_stock_1 << 1) | (kill_stock_2 << 2) | (kill_stock_3 << 3);

            *(h2p_lw_parser_addr + 14) = parser_control_word;

            if (current_kill_switch_state != last_kill_switch_state) {
                printf("[INFO] Kill switches: stock1(INTC)=%lu stock2(MSFT)=%lu stock3(AAPL)=%lu\n",
                       kill_stock_1, kill_stock_2, kill_stock_3);
