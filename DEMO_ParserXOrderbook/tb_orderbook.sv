...
module tb_orderbook();
    ...
    initial begin
        ...
        send_op(BID, 0, ADD, 104, 2); 
        send_op(BID, 1, ADD, 105, 4);
        send_op(BID, 2, ADD, 104, 3); 
        send_op(BID, 3, ADD, 107, 1);
        send_op(BID, 3, DELETE, 0, 0);
        send_op(BID, 1, EXECUTE, 0, 2);
        send_op(BID, 1, DELETE, 0, 0);
        send_op(BID, 4, ADD, 108, 4);
        send_op(BID, 5, ADD, 108, 4);
        send_op(BID, 6, ADD, 107, 3);
        send_op(BID, 6, EXECUTE, 0, 1);
        send_op(BID, 6, CANCEL, 0, 1);
        send_op(BID, 6, CANCEL, 0, 1);
