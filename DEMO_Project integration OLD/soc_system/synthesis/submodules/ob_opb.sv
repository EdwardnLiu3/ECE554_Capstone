...
(* ramstyle = "M10K" *) ob_packet_t OPB [0:OPB_DEPTH-1];
        ...
        if(p_add) begin
            ...
            if((is_cancel || is_execute) && (p_order_id == i_order_id) && (i_valid)) begin
                ...
                if (packet_out.quantity <= (quantity_to_remove + p_quantity)) begin
                    OPB[p_order_id] <= '{price: packet_out.price, quantity: '0, side: packet_out.side};
                end else begin
                    OPB[p_order_id] <= '{price: packet_out.price, quantity: packet_out.quantity - quantity_to_remove - p_quantity, side: packet_out.side};
