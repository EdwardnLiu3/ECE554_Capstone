...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (valread > 0) {
                printf("\n[RECEIVED] '%s'\n", buffer);
                int number = extract_last_integer(buffer);
                if (number != -1) {
                    printf("           -> Parsed integer: %d. Updating FPGA...\n", number);
                    current_LED_state = number;
                    LED_Set(number);
                    SEG7_Decimal(number);
                }
            } else if (valread == 0) {
            ...
            if (current_button_state != last_button_state) {
                for (int i=0; i<4; i++) {
                    ...
                    if (((last_button_state >> i) & 1) && !((current_button_state >> i) & 1)) {
                        printf("[EVENT] FPGA KEY%d Pressed! Sending data back to laptop... (Value: %d)\n", i, current_LED_state);
                        char send_buf[256];
                        sprintf(send_buf, "FPGA_MSG: KEY%d Pressed. Current state is %d\n", i, current_LED_state);
